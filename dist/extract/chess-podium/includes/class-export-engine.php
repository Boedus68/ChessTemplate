...
final class ChessPodium_ExportEngine
{
    ...
    public static function generate_export(object $tournament, array $players, array $standings, array $allPairings, array $photos, array $pgns): array
    {
        ...
        $navHtml = self::build_nav_html($slug, $roundsTotal);

        $indexHtml = self::build_index_html($tournament, $players, $standings, $navHtml, $playerStats);
    ...
    private static function compute_player_stats(array $players, array $allPairings): array
    {
        ...
        foreach ($playerMap as $pid => $stats) {
            ...
            $points = $stats['wins'] + 0.5 * $stats['draws'] + $byePoints * $stats['byes'];
            $perf = $stats['rating'];
    ...
    private static function player_link(string $name, int $playerId, array $playerStats, string $country = ''): string
    {
        $flag = $country ? ChessPodium_CountryHelper::render_flag_img($country, $country) . ' ' : '';
        if (!isset($playerStats[$playerId])) {
            return $flag . esc_html($name);
        }
        $slug = 'cp-player-' . $playerId;
        return $flag . '<a href="#' . esc_attr($slug) . '" class="cp-player-link" data-player-id="' . (int) $playerId . '">' . esc_html($name) . '</a>';
    }
...
(function() {
    ...
    private static function build_nav_html(string $slug, int $roundsTotal): string
    {
        $links = [
        ...
        $items = '';
        foreach ($links as $l) {
            $items .= '<li><a href="' . esc_attr($l[0]) . '">' . esc_html($l[1]) . '</a></li>';
        }
        return '<nav class="nav"><ul>' . $items . '</ul></nav>';
    }

    private static function build_index_html(object $tournament, array $players, array $standings, string $navHtml, array $playerStats): string
    {
        ...
        foreach ($players as $i => $p) {
            $pid = (int) $p->id;
            $country = isset($p->country) ? (string) $p->country : '';
            $link = self::player_link((string) $p->name, $pid, $playerStats, $country);
        ...
        foreach ($top3 as $i => $s) {
            ...
            $link = self::player_link((string) $s['name'], $pid, $playerStats, $country);
        ...
        $content = '<h2>' . esc_html__('Players', 'chess-podium') . '</h2><table class="table"><thead><tr><th>#</th><th>' . esc_html__('Name', 'chess-podium') . '</th><th>' . esc_html__('Rating', 'chess-podium') . '</th></tr></thead><tbody>' . $rows . '</tbody></table>' .
            ...
            '<p><a href="standing.html">' . esc_html__('View full standings', 'chess-podium') . '</a> | <a href="games.html">' . esc_html__('PGN games', 'chess-podium') . '</a> | <a href="galleria.html">' . esc_html__('Photo gallery', 'chess-podium') . '</a></p>';
    ...
    private static function build_pairs_html(object $tournament, array $pairings, int $round, string $navHtml, array $playerStats): string
    {
        ...
        foreach ($pairings as $p) {
            ...
            $whiteLink = self::player_link((string) $p->white_name, $wId, $playerStats, $whiteCountry);
            $blackLink = $isBye ? '-' : self::player_link((string) $p->black_name, $bId, $playerStats, $blackCountry);
    ...
    private static function build_games_html(object $tournament, array $allPairings, array $pgns, string $navHtml): string
    {
        ...
        foreach ($pgns as $pgn) {
            ...
            $gamesList .= '<div class="game-block" id="game-wrap-' . $gameId . '"><h4>' . sprintf(__('Round %d', 'chess-podium'), $round) . ': ' . $white . ' - ' . $black . ' ' . $result . '</h4>';
            $gamesList .= '<script type="text/plain" id="pgn-' . $gameId . '">' . preg_replace('/<\/script>/', '<\\/script>', $pgnContent) . '</script>';
            $gamesList .= '<div class="board-container" id="board-' . $gameId . '"></div><div class="pgn-controls"></div><div class="pgn-moves"></div></div>';
        }
        if ($gamesList === '') {
        ...
        return self::wrap_html(
            ...
            $navHtml,
            '<h2>' . esc_html__('PGN games', 'chess-podium') . '</h2><p>' . esc_html__('Use the controls below the board to navigate the game.', 'chess-podium') . '</p>' . $gamesList . $pgnViewerJs,
            true
        );
    }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    blocks.forEach(function(block, idx) {
        ...
        try {
            ...
            var movesHtml = moves.map(function(m, i) { return (i % 2 === 0 ? (i/2 + 1) + ". " : "") + m; }).join(" ");
            var movesEl = block.querySelector(".pgn-moves");
            if (movesEl) movesEl.innerHTML = "<strong>' . esc_js(__('Moves:', 'chess-podium')) . '</strong> " + (movesHtml || "' . esc_js(__('(no moves)', 'chess-podium')) . '");
            var moveIdx = 0;
            var ctrlEl = block.querySelector(".pgn-controls");
            if (ctrlEl) {
                var prevBtn = document.createElement("button");
                prevBtn.textContent = "' . esc_js(__('← Previous', 'chess-podium')) . '";
                prevBtn.onclick = function() {
                    if (moveIdx > 0) { moveIdx--; game.reset(); for (var j = 0; j < moveIdx; j++) game.move(moves[j]); board.position(game.fen()); }
                };
                var nextBtn = document.createElement("button");
                nextBtn.textContent = "' . esc_js(__('Next →', 'chess-podium')) . '";
                nextBtn.onclick = function() {
                    if (moveIdx < moves.length) { game.move(moves[moveIdx]); moveIdx++; board.position(game.fen()); }
                };
    ...
    private static function wrap_html(string $title, string $navHtml, string $content, bool $extraScripts = false): string
    {
        ...
        $lang = get_locale();
        return '<!DOCTYPE html><html lang="' . esc_attr(substr($lang, 0, 2)) . '"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' . $title . '</title>' . $css . '</head><body><header>' . $navHtml . '</header><main class="container"><h1>' . $title . '</h1>' . $content . '</main><footer><p>' . $footer . '</p></footer>' . $scripts . '</body></html>';
    }
...
.game-block { margin: 2rem 0; padding: 1rem; border: 1px solid #ddd; }
.board-container { width: 400px; max-width: 100%; }
footer { text-align: center; padding: 1rem; color: #666; font-size: 0.9em; }
button { margin: 0.25rem; padding: 0.5rem 1rem; cursor: pointer; }
.cp-player-link { color: #2980b9; text-decoration: none; }
